...
import com.beiing.autonewlinelayout.R;
...
public class AutoNewLineLayout extends ViewGroup {
     ...
     * 两个子控件之间的横向间隙
     */
    protected float horizonalSpace = 0;
    ...
    public AutoNewLineLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        horizonalSpace = ta.getDimension(R.styleable.AutoNewLineLayout_horizonalSpace, 0);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if(rawWidth + childWidth  + (rowIndex > 0 ? horizonalSpace : 0)> widthSpecSize - getPaddingLeft() - getPaddingRight()){
                //换行
                width = Math.max(width, rawWidth);
                rawWidth = childWidth;
...
//        //1. width=908, height=172  2.width=858, height=162
